...
- Traverses a ROOT directory (plain .html/.htm files, .zip archives containing html, and gzipped html)
- You can also point ROOT directly at a single HTML/ZIP/GZIP file and it will be processed.
...
def pick_root_if_needed(root):
    ...
    try:
        ...
        if folder and os.path.isdir(folder):
            return folder
        file_path = filedialog.askopenfilename(title="Or pick an HTML/ZIP/GZIP file to process", filetypes=[
            ("HTML", "*.html *.htm"),
            ("ZIP", "*.zip"),
            ("GZIP", "*.gz"),
            ("All", "*.*"),
    ...
    while True:
        try:
            folder = input("Enter folder/file path to scan (or leave blank to quit): ").strip('"').strip()
        except EOFError:
            folder = ""
        if not folder:
            ...
            with zipfile.ZipFile(io.BytesIO(blob)) as z:
                for name in z.namelist():
                    if not name.lower().endswith(TEXT_EXTS):
                        continue
                    try:
                        yield f"{path}|{name}", z.read(name).decode("utf-8", "ignore")
                    except Exception:
                        continue
        except Exception:
            return
...
def iter_html_payloads(root):
    if os.path.isfile(root):
        for item in _iter_from_file(root) or []:
            yield item
        return
    ...
    for dirpath, _, filenames in os.walk(root):
        for fn in filenames:
            ...
            for item in _iter_from_file(path) or []:
                yield item
...
def run():
    ...
    scan_target = root
    output_dir = root if os.path.isdir(root) else (os.path.dirname(root) or os.getcwd())
    ...
    for name, html in iter_html_payloads(scan_target):
        seen += 1
        try:
    ...
    out_csv = os.path.join(output_dir, OUT_BASENAME)
    fieldnames = [
        "file",
        "url",
        "ad_id",
